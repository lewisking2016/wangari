<?php
/**
 * Sub-Module: Breeding Tab Content
 */
...
if (!function_exists('renderInputBreedingLocal')) {
    function renderInputBreedingLocal(string $label, string $name, string $value = '', string $type = 'text'): string {
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:16px;">
    ...
    <a href="?tab=breeding&action=add" class="btn btn-primary" style="border-radius:4px;display:inline-flex;align-items:center;gap:8px;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:right;">
                        <a class="btn btn-outline btn-sm" href="?tab=breeding&action=view&id=<?php echo (int)$item['id']; ?>">View</a>
                        <a class="btn btn-outline btn-sm" href="?tab=breeding&action=edit&id=<?php echo (int)$item['id']; ?>">Edit</a>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php endif; ?>
...
<div class="admin-card">
    ...
    <form method="POST" action="?tab=breeding&action=<?php echo $action; ?><?php echo $action === 'edit' ? '&id=' . $selectedRecord['id'] : ''; ?>">
        ...
        <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px;">
            <?php echo renderInputBreedingLocal('Animal / Herd', 'subject', $formValues['subject']); ?>
            <?php echo renderInputBreedingLocal('Type', 'type', $formValues['type']); ?>
            <?php echo renderInputBreedingLocal('Male Parent', 'male_parent', $formValues['male_parent']); ?>
        ...
        <div style="margin-top:20px;display:flex;gap:12px;flex-wrap:wrap;">
            ...
            <a href="?tab=breeding" class="btn btn-outline" style="border-radius:4px;">Cancel</a>
        </div>
    </form>
</div>
...
<div class="admin-card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        <div>
            <h3 style="margin:0;font-family:'Outfit',sans-serif;font-size:1.2rem;color:var(--admin-text-heading);">Breeding Record Details</h3>
            <p style="margin:6px 0 0 0;color:#64748b;">View breeding details and due date.</p>
        </div>
        <a href="?tab=breeding" class="btn btn-outline" style="border-radius:4px;">Back</a>
    </div>
