...
export default function AdminFarmsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-white">Farms</h1>
        <p className="mt-1 text-sm text-slate-400">All tenant farms with owner, plan state, and workforce size.</p>
      </div>
      ...
      {!rows ? (
        <div className="animate-pulse text-sm text-slate-400">Loading farms…</div>
      ) : (
              ...
              {rows.map((f) => (
                <tr key={f.id}>
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-slate-500">{f.code || "no code"}{f.county ? ` · ${f.county}` : ""}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-slate-300">{f.owner?.name || "—"}</div>
                    <div className="text-xs text-slate-500">{f.owner?.email}</div>
                  </td>
                  <td className="px-4 py-3 text-slate-300">{f.workers}</td>
                  <td className="px-4 py-3 text-slate-300">{f.flocks}</td>
                  <td className="px-4 py-3">
                    {f.plan ? (
                      <div>
                        <span className="inline-flex rounded-full bg-emerald-500/15 px-2 py-0.5 text-[11px] font-medium text-emerald-400">{f.plan.name}</span>
                        <div className="mt-0.5 text-[11px] text-slate-500">till {new Date(f.plan.expiresAt).toLocaleDateString()}</div>
                      </div>
                    ) : (
                      <span className="text-xs text-slate-500">trial / none</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => extend(f)} className="rounded-lg px-2.5 py-1 text-xs text-slate-300 hover:bg-slate-800 hover:text-white">Extend…</button>
                  </td>
                </tr>
              ))}
              {rows.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-slate-500">No farms match.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
